...
if len(args.split):
    ...
    results_dir = args.run_dir + f"/ASFormer/results/{args.test_subset}/split_" + args.split + f"_{num_decoders}d_layer{num_layers}"
else:
    vid_list_file_tst = args.run_dir + f"/data/splits/{args.test_subset}.bundle"
    mapping_file = args.run_dir + "/data/mapping.txt"
    results_dir = args.run_dir + f"/ASFormer/results/{args.test_subset}/all" + f"_{num_decoders}d_layer{num_layers}"
...
trainer.predict(args.checkpoint, results_dir, features_path, batch_gen_tst, num_epochs, actions_dict, sample_rate)
